...
1.<u>transcribe_audio</u>: This takes the raw audio bytes from the microphone and sends them to Groq's Whisper model to get text.
...
•	<u>Dynamic Context</u>: The prompt injects the Target Role (e.g., Software Engineer) and Experience Level so the AI adapts its persona.
•	<u>State Management</u>: The agent tracks which stage it is in: Introduction, Hard Skills, Soft Skills, or Feedback.
•	<u>Hidden Triggers</u>: I instructed the LLM to output hidden tokens like MOVING_TO_HARD_SKILLS. *My Python code detects these tokens to automatically advance the interview stage without the user needing to do anything.*
...
•	<u>Configuration</u>: Where users set their target role.
